...
- Send the Song lets you preview the song direction first, then refine lyrics, genre, and emotional tone before you commit.
...
## How Send the Song fits this search intent

Send the Song is a strong fit for this keyword because the product matches the emotional job the reader is trying to get done. Most people searching for a mother-to-son song want one of two outcomes:

1. Find a song that already exists.
2. Create a song that sounds like it was made for their son.

Send the Song supports the second path with a much lower-friction workflow:
...
At Send the Song, the current entry points are:
...
async function main() {
  ...
  try {
    ...
    const description =
      "Create a song from a mother to her son with personal memories, lyric prompts, mother-son dance ideas, and Send the Song's free preview workflow.";
