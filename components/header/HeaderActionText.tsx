...
export function HeaderActionText({ children, icon }: HeaderActionTextProps) {
  return (
    <span className="relative inline-block overflow-hidden align-middle leading-[1.15]">
      ...
      <span
        aria-hidden="true"
        className="absolute inset-x-0 top-0 flex -translate-y-[120%] items-center gap-2 text-primary-foreground transition-transform duration-500 ease-[cubic-bezier(0.22,1.38,0.36,1)] motion-reduce:transition-none group-hover/nav-link:translate-y-0 group-focus-visible/nav-link:translate-y-0"
      >
        {icon}
        {children}
      </span>
